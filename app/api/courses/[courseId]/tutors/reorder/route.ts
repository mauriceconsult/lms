...
export async function PATCH(
  ...
  try {
    ...
    // Verify all tutor IDs belong to the course
    const tutorIds = list.map((item) => item.id);
    const validTutors = await db.tutor.findMany({
      where: {
        id: { in: tutorIds },
        courseId,
      },
    ...
    if (validTutors.length !== list.length) {
      return new NextResponse(
        JSON.stringify({
          success: false,
          message: "Invalid tutor IDs: Some IDs do not belong to this course",
        }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    return new NextResponse(
      JSON.stringify({
        success: true,
        message: "Tutors reordered successfully",
      }),
