...
export async function PATCH(
  ...
  try {
    ...
    // Verify all assignment IDs belong to the course
    ...
    if (validAssignments.length !== list.length) {
      return new NextResponse(
        JSON.stringify({
          success: false,
          message: "Invalid assignment IDs: Some IDs do not belong to this course",
        }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
