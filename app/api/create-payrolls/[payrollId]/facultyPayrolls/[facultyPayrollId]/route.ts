...
export async function DELETE(
  ...
  try {
    ...
    const deletedCourse = await db.facultyPayroll.delete({
      where: {
        id: params.facultyPayrollId,
      },
    });
    ...
    console.log("[FACULTY_PAYROLL_ID_DELETE]", error);
...
export async function PATCH(
  ...
  try {
    const { userId } = await auth();
    const { payrollId, facultyPayrollId } = params;
    ...
    console.log("[FACULTY_PAYROLL_ID]", error);
    return new NextResponse("Internal Error", { status: 500 });
  }
}
