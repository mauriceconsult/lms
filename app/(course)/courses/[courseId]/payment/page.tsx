...
const TuitionPaymentPage = () => {
  const [formData, setFormData] = useState<FormData>({ partyId: "" });
  ...
  useEffect(() => {
    console.log(
      `[${new Date().toISOString()} TuitionPaymentPage] Params:`,
      params
    );
    ...
    const fetchCourse = async (retryCount = 0) => {
      try {
        const url = `/api/courses/${params.courseId}`;
        console.log(
          `[${new Date().toISOString()} TuitionPaymentPage] Fetching course from:`,
          url
        );
        ...
        console.log(
          `[${new Date().toISOString()} TuitionPaymentPage] Fetch status: ${
            response.status
          }`
        );
        if (!response.ok) {
          const errorText = await response.text();
          const errorMessage = `Course fetch failed: ${response.status} ${
          ...
          if (response.status >= 500 && retryCount < 2) {
            console.log(
              `[${new Date().toISOString()} TuitionPaymentPage] Retrying (${
                retryCount + 1
              }/2)...`
            );
        ...
        console.log(
          `[${new Date().toISOString()} TuitionPaymentPage] Course response:`,
        ...
        console.log(
          `[${new Date().toISOString()} TuitionPaymentPage] Parsed amount: ${amountNum}`
        );
        if (!result.data.amount || isNaN(amountNum) || amountNum <= 0) {
          throw new Error(
            `Course amount is invalid or missing: ${result.data.amount}`
          );
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.partyId.match(/^256\d{9}$/)) {
    ...
    try {
      ...
      console.log(
        `[${new Date().toISOString()} TuitionPaymentPage] Tuition creation response:`,
        JSON.stringify(result, null, 2)
      );
      if (!response.ok || !result.success) {
        throw new Error(result.message || "Failed to create tuition");
      }
      // Create userProgress record for enrollment
      await fetch(`/api/courses/${params.courseId}/enrollment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
  ...
  const handlePayNow = () => {
    ...
    console.log(
      `[${new Date().toISOString()} TuitionPaymentPage] Navigating to /courses/${
        params.courseId
      }/payment/options?facultyId=${course.facultyId}`
    );
    router.push(
