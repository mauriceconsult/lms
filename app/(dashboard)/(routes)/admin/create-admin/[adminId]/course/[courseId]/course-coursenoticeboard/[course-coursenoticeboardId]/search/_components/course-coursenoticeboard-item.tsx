...
export const CourseNoticeboardItem = ({
    label,
    value,
    // icon: Icon,
}: CourseNoticeboardItemProps) => {
    const pathname = usePathname();
    const router = useRouter();
    const searchParams = useSearchParams();
    const currentCourseNoticeboardId = searchParams.get("courseId");
    const currentTitle = searchParams.get("title");
    const isSelected = currentCourseNoticeboardId === value;
    const onClick = () => {
