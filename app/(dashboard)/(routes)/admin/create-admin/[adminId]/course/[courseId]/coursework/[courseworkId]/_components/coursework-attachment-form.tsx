...
export const CourseworkAttachmentForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post(`/api/create-admins/${adminId}/courses/${courseId}/courseworks/${courseworkId}/attachments`, values);
  ...
  const onDelete = async (id: string) => {
    try {
      setDeletingId(id);
      await axios.delete(
        `/api/create-admins/${adminId}/courses/${courseId}/courseworks/${courseworkId}/attachments/${id}`
      );
        ...
        <>
          {initialData.attachments.length === 0 && (
            <p className="text-sm mt-2 text-slate-500 italic">
              You may add attachments to support your students in the Coursework
            </p>
          )}
          {initialData.attachments.length > 0 && (
