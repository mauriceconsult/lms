...
const Preview = ({ value }: { value: string | null }) => {
  ...
  if (!plainText)
    return <p className="text-slate-500 italic">Provide an assignment for your uploaded tutorial.</p>;
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const response = await fetch(
        `/api/create-admins/${adminId}/courses/${courseId}/tutorials/${tutorialId}/assignments/${assignmentId}/descriptions`,
        {
          method: "PATCH",
