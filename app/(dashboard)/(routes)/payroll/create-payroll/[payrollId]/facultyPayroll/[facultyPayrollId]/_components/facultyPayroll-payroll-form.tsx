...
interface FacultyPayrollFormProps {
...
export const FacultyPayrollForm = ({
  initialData,
  payrollId,
  facultyPayrollId,
  options,
}: FacultyPayrollFormProps) => {
  ...
  const selectedOption = options.find(
    (option) => option.value === initialData.payrollId
  );
  ...
  return (
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && (
        ...
        >
          {selectedOption?.label || "No Payroll"}
        </p>
      )}
      {isEditing && (
        <Form {...form}>
